...
export interface VendorProfile {
  ...
  account_type?: 'organization' | 'vendor';
}
...
class VendorService {
  ...
  // VendorIQ Assessment Functions - Renamed to avoid duplicate
  async createVendorIQAssessment(input: {
    vendor_id: string;
    framework: string;
    assessment_type: 'initial' | 'annual' | 'incident' | 'compliance';
    status: 'not-started' | 'in-progress' | 'completed' | 'overdue';
    score?: number;
    nist_mappings?: Record<string, any>;
  ...
  async getLatestVendorAssessment(
    vendor_id: string,
    framework: string = 'VendorIQ'
  ...
  async getVendorAssessmentsByFramework(
    framework: string
  ...
  // Note: createVendorProfile method already exists at line 85
  // This duplicate was removed to fix build error

  /**
   * Update service types
   */
  async updateServiceTypes(vendorId: string, serviceTypes: string[]): Promise<void> {
    try {
      const { error } = await supabase
        .from('vs_vendor_profiles')
      ...
      logger.error('Error updating service types:', error);
      throw error;
    }
  }
   ...
   * Update data types
   */
  async updateDataTypes(vendorId: string, dataTypes: string[]): Promise<void> {
    try {
      const { error } = await supabase
        .from('vs_vendor_profiles')
        .update({ data_types_accessed: dataTypes })
        .eq('id', vendorId);

      if (error) throw error;
    } catch (error) {
      logger.error('Error updating data types:', error);
      throw error;
    }
  }
   ...
   */
  async enableDirectoryListing(vendorId: string, enabled: boolean): Promise<void> {
    try {
      const { error } = await supabase
        .from('vs_vendor_profiles')
        .update({
          directory_listing_enabled: enabled,
          is_public: enabled ? true : undefined,
        })
        .eq('id', vendorId);

      if (error) throw error;
    } catch (error) {
      logger.error('Error enabling directory listing:', error);
      throw error;
    }
  }
   ...
   * Get public profile
   */
  async getPublicProfile(vendorId: string): Promise<VendorProfile | null> {
    try {
      const { data, error } = await supabase
        .from('vs_vendor_profiles')
        .select('*')
