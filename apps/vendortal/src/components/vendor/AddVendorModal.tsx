...
import { useVendorEnforcement } from '../../hooks/useUsageEnforcement';
...
const AddVendorModal: React.FC<AddVendorModalProps> = ({ onClose, onSuccess }) => {
  const { user } = useAuth();
  const { enforceUsage, UsageUpgradeModal } = useVendorEnforcement();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Check usage limit before creating vendor
    if (!enforceUsage()) {
