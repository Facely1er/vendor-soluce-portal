...
const VendorOnboardingWizard: React.FC<VendorOnboardingWizardProps> = ({ onComplete }) => {
  ...
  const handleInputChange = (field: string, value: any) => {
    if (field.includes('.')) {
      ...
      setFormData(prev => ({
        ...prev,
        [parent]: {
          ...prev[parent as keyof VendorOnboardingData],
          [child]: value
        }
      }));
    } else {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      logger.info('Submitting vendor onboarding data:', formData);
                    ...
                    <div className="text-center">
                      {uploading[doc.key] ? (
                        <div className="flex flex-col items-center">
                          ...
                          <p className="text-sm text-vendortal-navy font-medium">Uploading...</p>
                        </div>
                      ) : (
                        <>
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-gray-600 dark:text-gray-300">
            Complete your vendor registration to start using VendorTal
          </p>
        </div>
              ...
              return (
                <div key={step.id} className="flex items-center">
                  <div className={`flex items-center justify-center w-10 h-10 rounded-full border-2 ${
                    isCompleted 
                      ? 'bg-vendortal-purple border-vendortal-purple text-white'
                      : isActive
                      ? 'border-vendortal-navy bg-vendortal-navy text-white'
                  ...
                  {index < steps.length - 1 && (
                    <div className={`w-16 h-0.5 mx-2 ${
                      isCompleted ? 'bg-vendortal-purple' : 'bg-gray-300 dark:bg-gray-600'
                    }`} />
                  )}
                </div>
              );
