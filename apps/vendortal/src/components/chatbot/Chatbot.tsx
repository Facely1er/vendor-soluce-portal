...
const Chatbot: React.FC<ChatbotProps> = ({ 
  ...
  const knowledgeBase = {
    ...
    'general': {
      ...
      topics: {
        ...
        'support': {
          response: "We're here to help you succeed with VendorTal:\n\n**Support Channels:**\n• **In-App Chat**: This chatbot for immediate assistance\n• **Documentation**: Comprehensive guides and tutorials\n• **Email Support**: support@vendortal.com\n• **Knowledge Base**: Searchable help articles\n• **Video Tutorials**: Step-by-step walkthroughs\n\n**Resources:**\n• User Guide\n• API Documentation\n• Integration Guides\n• Best Practices",
          suggestions: [
  ...
  // Initialize with welcome message - use a ref to track initialization
  ...
  useEffect(() => {
    if (!initializedRef.current && messages.length === 0) {
      ...
      const randomGreeting = contextData.greetings[0];
      ...
      // Disable eslint for this legitimate one-time initialization
  ...
  const handleSendMessage = async () => {
    ...
    // Simulate API call delay with deterministic value
    const delay = 1500; // Fixed delay for consistency
    setTimeout(() => {
  ...
  const generateBotResponse = (userInput: string): { content: string; suggestions?: string[]; attachments?: any[] } => {
    ...
    if (context === 'vendor-onboarding') {
      ...
      return {
        content: "I can help you with VendorTal features and functionality. You can ask me about:\n\n• Core features and capabilities\n• Getting started guide\n• Support and documentation\n• Best practices for supply chain risk management\n\nWhat would you like to learn about?",
        suggestions: [
  ...
  return (
    <div className={`fixed bottom-4 right-4 z-50 ${isMinimized ? 'w-80' : 'w-96'} transition-all duration-300`}>
      <Card className={`shadow-2xl border-0 ${isMinimized ? 'h-16' : 'h-[600px]'} flex flex-col`}>
        <CardHeader className="pb-3 bg-vendortal-navy text-white rounded-t-lg">
          <div className="flex items-center justify-between">
            <div className="flex items-center">
              <Bot className="h-5 w-5 mr-2" />
              <CardTitle className="text-lg">VendorTal Assistant</CardTitle>
            </div>
              ...
              <div className="p-4 border-t border-gray-200 dark:border-gray-700">
                <div className="flex space-x-2">
                  <input
                    ...
                    className="flex-1 border border-gray-300 dark:border-gray-600 rounded-md px-3 py-2 text-sm bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-vendortal-navy"
                    disabled={isLoading}
                  />
                  <Button
