...
const EnhancedSBOMAnalysis: React.FC<EnhancedSBOMAnalysisProps> = ({ 
  ...
  const enhancedComponents: EnhancedComponent[] = components.map((comp, index) => {
    // Generate deterministic values based on component name for consistency
    const seed = comp.name.split('').reduce((acc, char) => acc + char.charCodeAt(0), 0);
    const downloadCount = (seed % 1000000) + 1000;
    const isDeprecated = (seed % 10) === 0; // 10% chance based on seed
  ...
  function generateMockVulnerabilities(componentName: string): VulnerabilityData[] {
    ...
    // Generate deterministic vulnerability based on component name
    const seed = componentName.split('').reduce((acc, char) => acc + char.charCodeAt(0), 0);
    if ((seed % 10) > 2) { // 70% chance based on seed
      vulns.push({
        cve: 'CVE-2024-67890',
        severity: 'medium',
        score: 5.3,
        ...
        <Card>
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              ...
              <FileJson className="h-8 w-8 text-vendortal-purple" />
            </div>
          </CardContent>
        </Card>
