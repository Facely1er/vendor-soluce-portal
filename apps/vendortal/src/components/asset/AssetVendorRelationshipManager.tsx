...
const AssetVendorRelationshipManager: React.FC<AssetVendorRelationshipManagerProps> = ({
  ...
  const loadData = useCallback(async () => {
    setLoading(true);
    try {
      const [assetData, relationshipsData, vendorsData, requirementsData] = await Promise.all([
        assetService.getAsset(assetId),
        assetService.getAssetVendorRelationships(assetId),
        vendorService.getVendorProfiles(),
        assetService.getDueDiligenceRequirements(assetId)
      ]);

      setAsset(assetData);
      setRelationships(relationshipsData);
      setVendors(vendorsData);
      setDueDiligenceRequirements(requirementsData);
    } catch (error) {
  ...
  const handleAddRelationship = async () => {
    ...
    try {
      ...
      await assetService.generateDueDiligenceRequirements(assetId, selectedVendor);
  ...
  const openEditModal = (relationship: AssetVendorRelationship) => {
    ...
    setIntegrationType(relationship.integration_type);
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center p-8">
        <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-vendortal-navy"></div>
      </div>
    );
  }
                  ...
                  <div key={relationship.id} className="border rounded-lg p-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-gray-900 dark:text-white">{relationship.integration_type.replace('_', ' ')}</p>
                      </div>
                    </div>
