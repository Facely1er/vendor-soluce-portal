...
const AssessmentSection: React.FC = () => {
  ...
  const getIcon = (iconName: string) => {
    const icons = {
      Shield: <Shield className="h-8 w-8 text-vendortal-purple" />,
      FileJson: <FileJson className="h-8 w-8 text-vendortal-purple" />,
      BarChart3: <BarChart3 className="h-8 w-8 text-vendortal-purple" />
    };
              ...
              <ul className="space-y-2 mb-6">
                {assessment.features.map((feature, index) => (
                  <li key={index} className="flex items-start">
                    <CheckCircle className="h-4 w-4 text-vendortal-purple mr-2 mt-0.5 flex-shrink-0" />
