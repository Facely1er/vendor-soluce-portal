...
const QuickAccessMenu: React.FC = () => {
  ...
  useEffect(() => {
    if (isAuthenticated) {
      // Check for old localStorage key for backward compatibility
      const oldStored = localStorage.getItem('vendorsoluce_recent_items');
      const stored = localStorage.getItem('vendortal_recent_items');
      if (stored) {
        try {
        ...
        try {
          const items = JSON.parse(oldStored);
          localStorage.setItem('vendortal_recent_items', oldStored);
          localStorage.removeItem('vendorsoluce_recent_items');
          setRecentItems(items.slice(0, 5));
        } catch (e) {
          logger.error('Error migrating recent items:', e);
        }
      }

      // Check for old localStorage key for backward compatibility
      const oldStoredFavorites = localStorage.getItem('vendorsoluce_favorites');
      const storedFavorites = localStorage.getItem('vendortal_favorites');
      if (storedFavorites) {
        try {
      ...
      } else if (oldStoredFavorites) {
        // Migrate from old key to new key
        try {
          const items = JSON.parse(oldStoredFavorites);
          localStorage.setItem('vendortal_favorites', oldStoredFavorites);
          localStorage.removeItem('vendorsoluce_favorites');
          setFavoriteItems(items);
        } catch (e) {
          logger.error('Error migrating favorites:', e);
        }
      }
    }
  }, [isAuthenticated]);
  ...
  const trackVisit = (label: string, path: string) => {
    ...
    localStorage.setItem('vendortal_recent_items', JSON.stringify(updatedItems));
  ...
  const toggleFavorite = (label: string, path: string) => {
    // Check for old localStorage key for backward compatibility
    const oldStored = localStorage.getItem('vendorsoluce_favorites');
    const stored = localStorage.getItem('vendortal_favorites');
    let items: QuickAccessItem[] = [];
    ...
    if (stored) {
      ...
      try {
        items = JSON.parse(oldStored);
        localStorage.setItem('vendortal_favorites', oldStored);
        localStorage.removeItem('vendorsoluce_favorites');
      } catch (e) {
        logger.error('Error migrating favorites:', e);
      }
    ...
    localStorage.setItem('vendortal_favorites', JSON.stringify(items));
    setFavoriteItems(items);
  };
  ...
  return (
    <div className="relative">
      <button
        ...
        className="p-2 text-gray-700 dark:text-gray-300 hover:text-vendortal-purple dark:hover:text-white hover:bg-gray-100 dark:hover:bg-gray-700 rounded-md transition-colors flex items-center"
