...
const Breadcrumbs: React.FC = () => {
  ...
  const routeLabels: Record<string, string> = {
    ...
    '/vendor-assessments': t('navigation.vendorAssessments') || 'Vendor Assessments',
  ...
  return (
    <nav className="flex items-center space-x-2 text-sm text-gray-600 dark:text-gray-400 mb-4" aria-label="Breadcrumb">
      <Link
        to="/"
        className="flex items-center hover:text-vendortal-purple dark:hover:text-vendortal-purple transition-colors"
        aria-label="Home"
      >
        ...
        return (
          <React.Fragment key={crumb.path}>
            ...
            {isLast ? (
              ...
              <Link
                to={crumb.path}
                className="hover:text-vendortal-purple dark:hover:text-vendortal-purple transition-colors"
              >
                {crumb.label}
              </Link>
            )}
