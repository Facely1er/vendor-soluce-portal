              ...
              {filteredCommands.map((command, index) => (
                <button
                  ...
                  className={`w-full px-4 py-3 flex items-center space-x-3 transition-colors ${
                    index === selectedIndex
                      ? 'bg-vendortal-purple/10 dark:bg-vendortal-purple/20'
                      : 'hover:bg-gray-100 dark:hover:bg-gray-700'
                  }`}
                >
                  <div className={`flex-shrink-0 ${index === selectedIndex ? 'text-vendortal-purple' : 'text-gray-500 dark:text-gray-400'}`}>
                    {command.icon}
                  </div>
                  <div className="flex-1 text-left">
                    <div className={`text-sm font-medium ${index === selectedIndex ? 'text-vendortal-purple dark:text-vendortal-purple' : 'text-gray-900 dark:text-white'}`}>
                      {command.label}
                    </div>
                    {command.description && (
