...
import { Send, Database, Shield, Eye, Lock, Globe, FileText, Users, AlertTriangle, Zap, Building, CheckCircle, Info } from 'lucide-react';
...
import SonarRadar from '../../components/vendor/SonarRadar';
import { exportVendorCSV, downloadCSV, parseVendorCSV, generateSampleVendorsCSV } from '../../utils/csvHelpers';
...
const VendorRiskRadar: React.FC = () => {
  ...
  const [sampleVendors, setSampleVendors] = useState<Array<{id: string, name: string, riskScore: number, riskLevel: 'Low' | 'Medium' | 'High' | 'Critical'}>>([]);
  
  // Add state for CSV import/export
  const handleExportCSV = () => {
    const vendorData = [{
      ...
      residualRisk: riskScore - 20,
      riskLevel: getRiskLevel(riskScore)
    }];
    const csv = exportVendorCSV(vendorData);
    downloadCSV(csv, `vendor-risk-export-${new Date().toISOString().split('T')[0]}.csv`);
  };
  ...
  const handleImportCSV = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        // Handle imported vendors
        logger.info('Imported vendors:', vendors);
      } catch (err) {
  ...
  const loadSampleData = () => {
    const csv = generateSampleVendorsCSV();
    downloadCSV(csv, 'sample-vendors.csv');
    
    // Also load sample vendors into radar
    const sampleData = [
      { id: '1', name: 'CloudTech Solutions', riskScore: 85, riskLevel: 'High' as const },
      { id: '2', name: 'DataGuard Inc', riskScore: 70, riskLevel: 'High' as const },
      { id: '3', name: 'SecurePayments Co', riskScore: 90, riskLevel: 'Critical' as const },
      { id: '4', name: 'Consulting Experts LLC', riskScore: 45, riskLevel: 'Medium' as const },
      { id: '5', name: 'Global Logistics Inc', riskScore: 65, riskLevel: 'Medium' as const }
    ];
    setSampleVendors(sampleData);
  };

  // Combine sample vendors with current vendor
  const allVendorsForRadar = sampleVendors.length > 0 
    ? sampleVendors 
    : vendorName && riskScore > 0 
      ? [{ id: 'current', name: vendorName, riskScore, riskLevel: getRiskLevel(riskScore) }]
      : [];
  ...
  const generateRadarData = (): RadarData[] => {
    if (!riskDimensions || riskDimensions.length === 0) {
      return [];
    }
    
    return riskDimensions.map(dimension => ({
      dimension: dimension.shortName || dimension.name || 'Unknown', // Fallback to name if shortName is missing
      risk: Math.max(0, Math.min(100, dimension.value * 20)), // Convert 1-5 scale to 0-100, ensure bounds
      required: Math.max(0, Math.min(100, getRequiredControlLevel(dimension.id) * 20)),
      fullMark: 100
    }));
  };
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                ...
                <p className="text-sm text-blue-800 dark:text-blue-300">
                  This radar displays risk scores from your VendorIQ assessments.
                  {sampleVendors.length === 0 && " Complete a "}
                  {sampleVendors.length === 0 && (
                    <Link to="/tools/vendor-iq?mode=quick" className="underline font-semibold">
                      Quick Assessment
                    </Link>
                  )}
                  {sampleVendors.length === 0 && " to add vendors to this view."}
                  {sampleVendors.length > 0 && " Manage your vendor portfolio risk at a glance."}
                </p>
              </div>
            </div>
          </CardContent>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
        ...
        {sampleVendors.length > 0 && (
          <div className="xl:col-span-3">
            <Card className={`${
              sampleVendors.filter(v => v.riskLevel === 'Critical').length > 0
                ? 'border-red-600 bg-red-50 dark:bg-red-900/20'
                : sampleVendors.filter(v => v.riskLevel === 'High').length > 0
                    ...
                    <>
                      ...
                      <div>
                        <p className="font-semibold text-red-900 dark:text-red-300">
                          Critical Alert: {sampleVendors.filter(v => v.riskLevel === 'Critical').length} Critical Risk Vendor{sampleVendors.filter(v => v.riskLevel === 'Critical').length > 1 ? 's' : ''}
                        </p>
                        <p className="text-sm text-red-700 dark:text-red-400">Immediate action required for high-risk vendors</p>
                      </div>
                    </>
                  ) : sampleVendors.filter(v => v.riskLevel === 'High').length > 0 ? (
                    <>
                      ...
                      <div>
                        <p className="font-semibold text-orange-900 dark:text-orange-300">
                          Warning: {sampleVendors.filter(v => v.riskLevel === 'High').length} High Risk Vendor{sampleVendors.filter(v => v.riskLevel === 'High').length > 1 ? 's' : ''}
                        </p>
                        <p className="text-sm text-orange-700 dark:text-orange-400">Review and monitoring recommended</p>
                      </div>
                    </>
                  ) : (
                    <>
                      ...
                      <div>
                        <p className="font-semibold text-purple-900 dark:text-purple-300">System Status: All Clear</p>
                        <p className="text-sm text-purple-700 dark:text-purple-400">No critical vendor risks detected</p>
                      </div>
                    </>
                  )}
                </div>
        ...
        {/* Stats Grid */}
        {sampleVendors.length > 0 && (
          <div className="xl:col-span-3 grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
            <Card>
              <CardContent className="p-4 text-center">
                <div className="text-2xl font-bold text-red-600">{sampleVendors.filter(v => v.riskLevel === 'Critical').length}</div>
                <div className="text-sm text-gray-500 dark:text-gray-400 mt-1">Critical</div>
              </CardContent>
            </Card>
            <Card>
              <CardContent className="p-4 text-center">
                <div className="text-2xl font-bold text-orange-600">{sampleVendors.filter(v => v.riskLevel === 'High').length}</div>
                <div className="text-sm text-gray-500 dark:text-gray-400 mt-1">High</div>
              </CardContent>
            </Card>
            <Card>
              <CardContent className="p-4 text-center">
                <div className="text-2xl font-bold text-blue-600">{sampleVendors.filter(v => v.riskLevel === 'Medium').length}</div>
                <div className="text-sm text-gray-500 dark:text-gray-400 mt-1">Medium</div>
              </CardContent>
            </Card>
            <Card>
              <CardContent className="p-4 text-center">
                <div className="text-2xl font-bold text-purple-600">{sampleVendors.filter(v => v.riskLevel === 'Low').length}</div>
                <div className="text-sm text-gray-500 dark:text-gray-400 mt-1">Low</div>
              </CardContent>
            </Card>
            <Card>
              <CardContent className="p-4 text-center">
                <div className="text-2xl font-bold text-gray-900 dark:text-white">{sampleVendors.length}</div>
            ...
            <Card>
              <CardContent className="p-4 text-center">
                <div className="text-2xl font-bold text-gray-900 dark:text-white">
                  {Math.round(sampleVendors.reduce((sum, v) => sum + v.riskScore, 0) / sampleVendors.length)}
                </div>
                <div className="text-sm text-gray-500 dark:text-gray-400 mt-1">Avg Risk</div>
              </CardContent>
            </Card>
          </div>
        )}
          ...
          <Card>
            ...
            <CardContent>
              <div className="flex gap-3">
                <Button variant="secondary" onClick={handleExportCSV} disabled={!vendorName.trim()}>
                ...
                <Button variant="ghost" onClick={() => setSampleVendors([])}>
                  Clear Vendors
                </Button>
              </div>
            </CardContent>
                      ...
                      <ul className="text-sm text-gray-600 dark:text-gray-400 space-y-1">
                        {getPrivacyRecommendations().slice(0, 3).map((rec, index) => (
                          <li key={index} className="flex items-start">
                            <span className="w-2 h-2 bg-vendortal-navy rounded-full mt-2 mr-2 flex-shrink-0"></span>
                            {rec}
                          </li>
                        ))}
                      </ul>
              ...
              <CardContent>
                <div className="space-y-4">
                  <div className="flex items-start">
                    <div className="bg-gray-100 dark:bg-gray-800 w-6 h-6 rounded-full flex items-center justify-center text-vendortal-navy dark:text-vendortal-purple mr-3 mt-0.5 flex-shrink-0">
                      1
                    </div>
