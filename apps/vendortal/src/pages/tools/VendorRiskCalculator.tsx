...
import { ArrowLeft, Send, Lock, Briefcase, Shield, Building, Server, AlertTriangle, Zap, ArrowRight, Info } from 'lucide-react';
...
import { LeadCaptureModal } from '../../components/freemium/LeadCaptureModal';
...
const VendorRiskCalculator: React.FC = () => {
  ...
  const [riskFactors, setRiskFactors] = useState<RiskFactor[]>([
    ...
    },
    {
      ...
      weight: 3,
      value: 1,
      icon: <Briefcase className="h-5 w-5 text-vendortal-purple" />
    },
    {
      ...
      weight: 3,
      value: 1,
      icon: <Shield className="h-5 w-5 text-vendortal-purple" />
    },
    {
      id: 'compliance',
    ...
    },
    {
      ...
      weight: 3,
      value: 1,
      icon: <Server className="h-5 w-5 text-vendortal-purple" />
    }
  ]);
  ...
  const calculateRiskScore = async () => {
    ...
    try {
      ...
      // Show lead capture for non-authenticated users
      if (!isAuthenticated) {
  ...
  const handleLeadCapture = async (email: string) => {
    ...
    try {
      ...
      // Don't block the user even if capture fails
    }
  };
  ...
  return (
    <div className="container mx-auto px-4 py-8">
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1">
            <p className="text-sm text-blue-800 dark:text-blue-300">
              <strong>VendorIQ Quick Assessment</strong> provides fast 5-minute risk screening. 
              For detailed analysis with NIST mapping, try our <Link to="/tools/vendor-iq?mode=standard" className="underline font-semibold">Standard Assessment</Link>.
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="mb-6">
        <Link to="/" className="inline-flex items-center text-gray-700 dark:text-gray-300 hover:text-vendortal-navy dark:hover:text-vendortal-purple transition-colors mb-4">
          <ArrowLeft className="mr-2 h-4 w-4" />
          {t('navigation.home')}
        </Link>
        <div className="flex items-center gap-2 mb-2">
          <Zap className="h-6 w-6 text-purple-500" />
          <h1 className="text-3xl font-bold text-gray-900 dark:text-white">VendorIQ Quick Assessment</h1>
          <span className="px-2 py-1 bg-purple-100 text-purple-700 dark:bg-purple-900/20 dark:text-purple-400 text-xs font-semibold rounded">FREE</span>
        </div>
        <p className="text-gray-600 dark:text-gray-300 mb-6">5-minute vendor risk screening for fast preliminary assessments</p>
      </div>
          ...
          {calculationComplete ? (
            <Card>
              ...
              <CardContent>
                ...
                {/* Upgrade CTA */}
                <Card className="mt-4 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-700">
                  <CardContent className="p-4">
                    <h4 className="font-semibold text-lg mb-2 text-gray-900 dark:text-white">Want More Details?</h4>
                    <p className="text-sm text-gray-600 dark:text-gray-400 mb-4">
                      Get comprehensive risk analysis with:
                    </p>
                    <ul className="text-sm text-gray-600 dark:text-gray-400 mb-4 space-y-1">
                      <li>• 6 detailed risk dimensions</li>
                      <li>• NIST 800-161 control mapping</li>
                      <li>• Compliance gap analysis</li>
                      <li>• Automated recommendations</li>
                    </ul>
                    <Link to="/tools/vendor-iq?mode=standard">
                      <Button variant="primary" className="w-full">
                        Upgrade to Standard Assessment
      ...
      <LeadCaptureModal
        ...
        title="Get Your Risk Assessment Report"
        description="Enter your email to receive a detailed risk assessment report with recommendations and next steps."
      />
    </div>
  );
};
