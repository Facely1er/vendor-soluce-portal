      ...
      (set: any, get: any): VendorState => ({
        ...
        updateVendor: async (id: string, vendorData: VendorUpdate, userId: string) => {
          ...
          try {
            const { data, error } = await supabase
              .from('vs_vendors')
              // @ts-ignore - Supabase type inference issue with update operation
              // eslint-disable-next-line @typescript-eslint/no-explicit-any
        ...
        selectVendor: (vendor: Vendor | null) => set({ selectedVendor: vendor }, false, 'selectVendor'),
        
        // Filter actions
        setSearchTerm: (term: string) => set({ searchTerm: term }, false, 'setSearchTerm'),
        setRiskLevelFilter: (level: string) => set({ riskLevelFilter: level }, false, 'setRiskLevelFilter'),
        setComplianceFilter: (status: string) => set({ complianceFilter: status }, false, 'setComplianceFilter'),
        setSorting: (sortBy: VendorState['sortBy'], sortOrder: VendorState['sortOrder']) => set({ sortBy, sortOrder }, false, 'setSorting'),
        clearFilters: () => set({
          searchTerm: '',
          riskLevelFilter: 'all',
          complianceFilter: 'all',
        ...
        getFilteredVendors: () => {
          ...
          if (state.riskLevelFilter !== 'all') {
            filtered = filtered.filter((vendor: Vendor) => vendor.risk_level === state.riskLevelFilter);
          }
          
          // Apply compliance filter
          if (state.complianceFilter !== 'all') {
            filtered = filtered.filter((vendor: Vendor) => vendor.compliance_status === state.complianceFilter);
          }
        ...
        getVendorById: (id: string) => {
          return get().vendors.find((vendor: Vendor) => vendor.id === id);
        },
        
        getRiskDistribution: () => {
          const vendors = get().vendors;
          return {
            critical: vendors.filter((v: Vendor) => v.risk_level === 'Critical').length,
            high: vendors.filter((v: Vendor) => v.risk_level === 'High').length,
            medium: vendors.filter((v: Vendor) => v.risk_level === 'Medium').length,
            low: vendors.filter((v: Vendor) => v.risk_level === 'Low').length,
          };
        },
        ...
        getComplianceStats: () => {
          const vendors = get().vendors;
          return {
            compliant: vendors.filter((v: Vendor) => v.compliance_status === 'Compliant').length,
            partial: vendors.filter((v: Vendor) => v.compliance_status === 'Partial').length,
            nonCompliant: vendors.filter((v: Vendor) => v.compliance_status === 'Non-Compliant').length,
          };
        },
      }),
      {
        name: 'vendortal-risk-review-vendor-store',
        // Only persist non-sensitive data
        partialize: (state: VendorState) => ({
