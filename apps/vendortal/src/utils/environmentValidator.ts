...
export function printValidationResults(result: ValidationResult): void {
  ...
  // Print summary
  logger.info('\n📊 Summary:');
  logger.info(`   Required variables: ${ENVIRONMENT_VARIABLES.filter(v => v.required).length - result.missingRequired.length}/${ENVIRONMENT_VARIABLES.filter(v => v.required).length} configured`);
  logger.info(`   Optional variables: ${ENVIRONMENT_VARIABLES.filter(v => !v.required).length - result.missingOptional.length}/${ENVIRONMENT_VARIABLES.filter(v => !v.required).length} configured`);
...
export function validateProductionEnvironment(): void {
  // Always run validation but handle errors differently based on environment
  const result = validateEnvironment();

  // Log results
  if (result.errors.length > 0) {
    if (import.meta.env.DEV) {
      logger.warn('Environment validation found issues in development mode', {
  ...
  // In development, just print results without throwing
  if (import.meta.env.DEV && (result.errors.length > 0 || result.warnings.length > 0)) {
    printValidationResults(result);
  }
}
