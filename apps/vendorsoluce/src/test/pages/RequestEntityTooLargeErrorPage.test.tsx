...
import RequestEntityTooLargeErrorPage from '../../pages/RequestEntityTooLargeErrorPage';
...
describe('RequestEntityTooLargeErrorPage', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it('renders request entity too large error page correctly', () => {
    render(
      <TestWrapper>
        <RequestEntityTooLargeErrorPage />
      </TestWrapper>
    );

    expect(screen.getByText('Request Entity Too Large')).toBeInTheDocument();
    expect(screen.getByText('The request entity is too large')).toBeInTheDocument();
  });
  ...
  it('displays troubleshooting steps', () => {
    ...
    expect(screen.getByText('Reduce file size')).toBeInTheDocument();
    expect(screen.getByText('Check upload limits')).toBeInTheDocument();
    expect(screen.getByText('Use compression')).toBeInTheDocument();
    expect(screen.getByText('Try smaller files')).toBeInTheDocument();
  });
  ...
  it('displays error code', () => {
    ...
    expect(screen.getByText('REQUEST_ENTITY_TOO_LARGE_ERROR')).toBeInTheDocument();
  });
  ...
  it('displays error prevention tips', () => {
    ...
    expect(screen.getByText('Check file sizes before upload')).toBeInTheDocument();
    expect(screen.getByText('Use compression when possible')).toBeInTheDocument();
    expect(screen.getByText('Split large files into smaller parts')).toBeInTheDocument();
  });
